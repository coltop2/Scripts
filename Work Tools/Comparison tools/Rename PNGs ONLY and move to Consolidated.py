...
def rename_files(directory_path):
    ...
    for file_name in file_list:
        ...
        elif is_png_file(file_path):  # Only process PNG files
            new_name = f"{parent_folder_name}_{file_name}"
...
def count_files(source_path):
    ...
    for root, _, files in os.walk(source_path):
        for filename in files:
            ...
            if is_png_file(file_path):  # Only count PNG files
                file_count += 1
    return file_count
...
def copy_files_to_destination(source_path, destination_path):
    ...
    for root, _, files in os.walk(source_path):
        for filename in files:
            ...
            if is_png_file(source_file):  # Only process PNG files
